...
def load_files(offsets = offsets, jsonfilename = jsonfilename, songfilename = songfilename):
  ...
  clips = [ VideoFileClip(name).subclip(offset) for name, offset in offsets.items() ]
...
def generate(clips, segments, song, name):
  ...
  clips = [ clips[i % len(clips)].subclip(*segment) for i, segment in tqdm(enumerate(segments)) ]
